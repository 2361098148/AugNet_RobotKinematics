...
class FeedForwardNet(nn.Module):
    ...
    def get_jacobian(self, x):
        ...
        n = x.size()[0]
        J = torch.zeros(self.out_size,self.in_size,n,device=x.device)  # n是样本数，out_size是输出维度，in_size是输入维度
        x.requires_grad = True
        y = self.forward(x)

        for j in range(self.out_size):
            g = torch.autograd.grad(y[:,j],x,retain_graph=True,grad_outputs=torch.ones(n,device=x.device))  # retain_graph=True保留计算图，grad_outputs=torch.ones(n)梯度张量，
            g = g[0].permute(1, 0)  # g[0]是一个元组，g[0].permute(1,0)是将g[0]的维度从（n，in_size）变为（in_size，n）
            g = torch.reshape(g,(1,self.in_size,n))  # 将g的维度从（in_size，n）变为（1，in_size，n）
            J[j,:,:] = g  # 将g赋值给J的第j行
